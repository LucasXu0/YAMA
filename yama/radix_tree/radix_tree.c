...
radix_tree_create()
{
	...
	kern_return_t kr =
			vm_allocate(mach_task_self(), &allocated, size, VM_FLAGS_ANYWHERE | VM_MAKE_TAG(VM_MEMORY_ANALYSIS_TOOL));
	if (kr != KERN_SUCCESS) {
		return NULL;
	}
...
radix_tree_grow(struct radix_tree **treep)
{
	...
	mach_vm_address_t allocated;
	kern_return_t kr =
			vm_allocate(mach_task_self(), &allocated, newsize, VM_FLAGS_ANYWHERE | VM_MAKE_TAG(VM_MEMORY_ANALYSIS_TOOL));
	if (kr != KERN_SUCCESS) {
		return;
	}
	D("GROW %p -> %p\n", *treep, (void *)allocated);
	kr = vm_copy(mach_task_self(), (mach_vm_address_t)*treep, size, allocated);
	if (kr != KERN_SUCCESS) {
		vm_deallocate(mach_task_self(), allocated, newsize);
		return;
	}
	uint32_t old_num_nodes = (*treep)->num_nodes;
	vm_deallocate(mach_task_self(), (mach_vm_address_t)*treep, size);
	*treep = (void *)allocated;
...
radix_tree_destory(struct radix_tree *tree)
{
	...
	assert(size % PAGE_SIZE == 0);
	vm_deallocate(mach_task_self(), (mach_vm_address_t)tree, size);
}
